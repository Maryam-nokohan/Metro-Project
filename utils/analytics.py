...
class OperationsLog:
    ...
    def record_trip(self, date: str, station_id: str, count: int = 1) -> None:
        ...
        self._records.append(TripRecord(date=date, station_id=station_id, trip_count=count))
    ...
    def average_daily_trips(self) -> float:
        ...
        for record in self._records:
            daily_totals[record.date] = daily_totals.get(record.date, 0) + record.trip_count
    ...
    def kth_busiest_station(self, k: int) -> Optional[Tuple[str, int]]:
        ...
        totals: Dict[str, int] = {}
        for record in self._records:
            totals[record.station_id] = totals.get(record.station_id, 0) + record.trip_count
